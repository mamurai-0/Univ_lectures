...
type expr =
  ...
  | ERecFun	   of int * (name * name * expr) list * env
  | EApp	   of expr * expr
...
let rec find_match : pattern -> value -> expr -> env -> (name * thunk) list option =
	fun p v e env ->
	match (p,v) with
	| (PInt(i1), VInt(i2)) -> if i1 = i2 then Some([]) else None
	| (PBool(b1), VBool(b2)) -> if b1 = b2 then Some([]) else None
	| (PVar(x), _) -> Some([x,Thunk(e,env)])
	| (PPair(p1,p2), VPair(v1,v2)) ->
		let f1 = find_match p1 v1 e env in
		let f2 = find_match p2 v2 e env in
		(match (f1,f2) with
