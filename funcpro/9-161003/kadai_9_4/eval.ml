...
let rec extend_closures closures env =
	let rec sub closures origin_closures env origin_env =
		...
		| (fi,xi,ei) :: closures' ->
			...
			sub closures' origin_closures (extend fi (Thunk((ERecFun(i, origin_closures, origin_env)), origin_env)) env) origin_env)
...
let rec eval_expr env e =
  ...
  | EApp (e1,e2) ->
	...
	let v1 = eval_expr env e1 in
	(match v1 with
	| VFun (x,ex,oenv) -> eval_expr ((x,Thunk(e2,env)) :: oenv) ex
	| VRecFun (i,closures,oenv) ->
		let (xi, ei) = search_xi_ei i closures in
		let env' = extend xi (Thunk(e2,env)) (extend_closures closures oenv) in
		eval_expr env' ei
  ...
  | EMatch(e1, l) ->
	...
	(match l with
	 ...
	 | (pi,ei) :: l' ->
	 	let result = find_match pi v e1 env in
	 	(match result  with
...
and print_thu thu =
	...
	| Thunk(e,env) ->
		(print_string "Thunk(";
		 ...
		 print_env env;
		 print_string ")")
	| _ -> raise (PrintErr "PrintErr in print_thu")
