...
let rec make_typeenv_4 : tyenv -> ty list -> constraints -> (name * name * expr) list -> tyenv =
	fun tenv ts cs closures ->
	let s = ty_unify cs in
	let s1s = List.map (ty_subst s) ts in
	let d = tenv_subst s tenv in
	make_generalize closures d s1s
...
let rec infer_expr : tyenv -> expr -> ty * constraints =
	...
	| EVar(x) -> 
		...
		| None -> raise (TypeErr ("Unbound value " ^ x))
	...
	| ELetRec(closures,e1) ->
		...
		let tenv_sub = make_typeenv_4 tenv ts cs closures in
